...
# Ci-dessous toutes les dépendances pour parser le fichier ODS
# installé avec pip
...
# ODS : nom du fichier contenant la liste des photos
FILE_INPUT='2018_BBNS_animationNoel_photos.ods'
FILE_SHEET='liste'
...
def parse_ods_file(file):
	...
	try:
		for row in data[ FILE_SHEET ][1:]:
			# normalement 6 colonnes, si plus on ignore
			if( len(row) >= 6 ):
				lret.append( (row[1], row[5]) )
			else:
				print("information manquante sur cette ligne :", row )
...
def main():
	...
	liste = parse_ods_file( FILE_INPUT )
	for idx, item in enumerate(liste):
		addr = item[0]
		photo = item[1]
		print("{2} Envoi à {0} la photo {1} : ".format(addr, photo, idx+1), end='')
		if Path(PHOTO_PATH + photo).is_file() :
			msg = create_message_with_attachment( 'Bébés Nageurs Stéphanois', addr, MAIL_SUBJECT, MAIL_BODY, PHOTO_PATH + photo)
